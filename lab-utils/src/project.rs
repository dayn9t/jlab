use lab_core::{Error, Label, LabelMeta, Result};
use std::fs;
...
impl Project {
    ...
    pub fn annotation_path(&self, image_name: &str) -> PathBuf {
        self.labels_dir().join(format!("{}.yaml", file_stem(image_name)))
    }
...
mod tests {
    ...
    fn delete_sample_removes_image_and_annotation() {
        ...
        project.save_annotation("a.jpg", &lab_core::new_label("test")).unwrap();
    ...
    fn merge_rejects_same_stem_with_different_extension() {
        ...
        project.save_annotation("b.jpg", &lab_core::new_label("existing")).unwrap();
