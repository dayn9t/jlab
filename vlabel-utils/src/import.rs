//! 外部数据集格式导入（YOLO / VOC / COCO / LabelMe → VLabel 项目）。
    ...
    for image_path in image_paths {
        ...
        if label_path.exists() {
            ...
            for line in content.lines() {
                ...
                if vlabel_core::find_category(meta, class_id).is_none() {
                ...
                objects.push(vlabel_core::new_object(0, class_id, polygon));
            }
        }
        ...
        if label_path.exists() {
            ...
            if let Some((width, height)) = parse_voc_size(&xml) {
                ...
                for voc_obj in voc_objects {
                    ...
                    objects.push(vlabel_core::new_object(0, category_id, polygon));
                }
            }
        }
...
pub fn import_from_coco(
    ...
    for cat in &dataset.categories {
        if vlabel_core::find_category(meta, cat.id).is_some() {
...
pub fn merge_imported_images(
    ...
    fs::create_dir_all(project.vlabels_dir())?;
    ...
    for item in imported {
        ...
        vlabel_core::io::save_annotation(&label_path, &item.annotation)?;
    }
...
pub fn build_label(objects: Vec<Object>, user_agent: &str) -> Label {
    let mut label = vlabel_core::new_label(user_agent);
...
pub(crate) mod tests {
    use super::*;
    use vlabel_core::{CatDef, RoiConfig, ShapeConfig};
    ...
    fn temp_root(tag: &str) -> PathBuf {
        let dir =
            std::env::temp_dir().join(format!("vlabel-utils-test-{}-{}", tag, std::process::id()));
