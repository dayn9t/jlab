...
//! theme, and UI settings (stored as JSON under `~/.config/vlabel/`).
//!
...
impl AppState {
    ...
    fn get_recent_projects_path(&self) -> anyhow::Result<PathBuf> {
        ...
        let config_dir = PathBuf::from(home).join(".config").join("vlabel");
    ...
    pub(crate) fn load_language_setting() -> Option<crate::i18n::Language> {
        ...
        let config_path = PathBuf::from(home).join(".config").join("vlabel").join("language.json");
    ...
    pub(crate) fn load_auto_save_setting() -> Option<bool> {
        let home = std::env::var("HOME").or_else(|_| std::env::var("USERPROFILE")).ok()?;
        let config_path = PathBuf::from(home).join(".config").join("vlabel").join("auto_save.json");
    ...
    pub(crate) fn load_theme_setting() -> Option<ThemeColor> {
        let home = std::env::var("HOME").or_else(|_| std::env::var("USERPROFILE")).ok()?;
        let config_path = PathBuf::from(home).join(".config").join("vlabel").join("theme.json");
    ...
    pub(crate) fn load_ui_settings() -> (Option<f32>, Option<f32>, Option<bool>) {
        ...
        let config_path =
            PathBuf::from(home).join(".config").join("vlabel").join("ui_settings.json");
...
mod tests {
    ...
    use std::fs;
    use std::path::PathBuf;
    use vlabel_core::{CatDef, LabelMeta, Point, Polygon, RoiConfig, ShapeConfig};
    ...
    fn temp_project(tag: &str, image_name: Option<&str>) -> PathBuf {
        let root =
            std::env::temp_dir().join(format!("vlabel-gui-state-{}-{}", tag, std::process::id()));
    ...
    fn redirect_home(tag: &str) -> PathBuf {
        let home =
            std::env::temp_dir().join(format!("vlabel-gui-home-{}-{}", tag, std::process::id()));
    ...
    fn load_project_resets_per_project_state() {
        ...
        state.clipboard_objects = vec![vlabel_core::new_object(0, 0, poly(0.1))];
