...
impl LabApp {
    pub(super) fn show_left_panel(&mut self, ctx: &Context) {
        ...
        if self.state.show_left_panel {
            SidePanel::left("left_panel").default_width(250.0).resizable(true).show(ctx, |ui| {
                let heading_size = ui
                    .style()
                    .text_styles
                ...
                if let Some(label) = &self.state.current_annotation {
                    ...
                    let selected_id = self.state.selected_object_id;
                    egui::ScrollArea::vertical().show(ui, |ui| {
                        for (idx, _roi) in label.rois.iter().enumerate() {
    ...
    pub(super) fn show_right_panel(&mut self, ctx: &Context) {
        ...
        if self.state.show_right_panel {
            SidePanel::right("right_panel").default_width(300.0).resizable(true).show(ctx, |ui| {
                let heading_size = ui
                    .style()
                    .text_styles
                    .get(&TextStyle::Heading)
                ...
                if let (Some(label), Some(selected_id), Some(meta)) =
                    (&mut self.state.current_annotation, selected_object_id, meta_clone.as_ref())
                {
                    // Single object editing mode
                    if let Some(obj) = label.objects.iter_mut().find(|o| o.id == selected_id) {
                        ...
                        ui.add_enabled_ui(can_edit_properties, |ui| {
                            ...
                            if let Some(category) = vlabel_core::find_category(meta, obj.category) {
                                        ...
                                        for prop_def in &category.properties {
                                                ...
                                                .find(|pt| pt.name == prop_def.property_type)
                                            {
                                                ...
                                                if ui
                                                    ...
                                                    .clicked()
                                                {
                                                    obj.properties.retain(|e| e.id != prop_def.id);
                                                    self.state.has_unsaved_changes = true;
                                                }
                                                ...
                                                if !meta.property_special_values.is_empty() {
                                                    ui.separator();
                                                    ui.label(
                                                        self.state.i18n.t("sidebar.special_values"),
                                                    );
